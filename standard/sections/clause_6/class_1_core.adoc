...
WMO-No. 1131 further requires support for observations from across all Earth system domains, necessitating
that a CDMS handle data from both fixed-position stations and mobile platforms.
...
[NOTE]
====
The definition of an observed element requires careful consideration of all dimensions that distinguish
one measurement from another, including: measurement type (e.g. temperature, pressure), sensor height or
depth, aggregation duration, statistical function (instantaneous, mean, maximum, minimum), and unit of
measure. Systems should ensure that element definitions are sufficiently granular to avoid accidental
overwriting of observations from different sensors or measurement configurations. Source data provenance
(e.g. transmission format or network) should be recorded separately rather than embedded in the element
definition.
====

[NOTE]
====
Support for observations from across the Earth system domains is a core requirement of this specification.
This includes both fixed-position stations and mobile platforms (ships, buoys, radiosondes, aircraft).
For fixed stations, geographic location is derived from station metadata. For mobile platforms, geographic
location must be stored with each individual observation.

"Earth system domains" encompasses the atmospheric, terrestrial, hydrological, cryospheric, and oceanic
environments from which in-situ point observations relevant to climate are collected. This specification
is not limited to atmospheric observations.
====
...
* Homogenisation provenance: break points detected with dates and statistical confidence levels; adjustment factors applied to each segment; reference series used; statistical significance of detected breaks; correspondence with known station changes; homogenised values stored as derived observations with both original and adjusted series retained
