...
|Statement a|
The system SHALL store complete observation-level provenance metadata for all observation data documenting the complete
data lineage including:
...
* Derived data lineage (source observations, algorithms or method identifier and version,
software identifier and version, parameters used in derivation)
...
* Backward traceability from a derived observation, dataset or product to its original source observations and processing events
...
|Statement a|
The system SHALL ensure provenance records follow an append-only model such that:

* Provenance entries cannot be modified or deleted after creation
* An erroneous provenance entry can be corrected by appending a new entry that corrects or supersedes
the erroneous entry, recording the reason for and timestamp of the correction
* The original provenance entry is preserved and remains retrievable alongside the corrected entry
* Original (raw) observation values are preserved even when corrections are applied
* A complete audit trail exists for all data modifications, including corrections applied to the provenance itself
|===

[[req_core_prov_automation]]
===== Provenance Metadata Automation

[cols="1,4"]
|===
2+^|**Requirement {counter:req-id}**
|Identifier|/req/core/provenance-automation
|Statement a|
* The system SHALL automatically generate and link provenance records when data ingestion, quality control, derivation,
homogenization, or other material processing activities occur.
* The records SHALL identify the relevant inputs, outputs, processing activity, timestamp, responsible system or agent,
and applicable method, software, configuration and version.
* Any manually added or amended provenance information SHALL be clearly identified and attributed.
|===
