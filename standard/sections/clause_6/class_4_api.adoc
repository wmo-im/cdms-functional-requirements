...
* Returns data in standard formats (JSON, CSV, NetCDF)
...
[NOTE]
====
The HTTP PATCH method is not listed as a requirement. Observational records in a CDMS are treated as
immutable once ingested; corrections are applied by creating new versioned records with audit trails
rather than partially updating existing ones. Implementations that require partial resource updates
for non-observational resources (e.g., metadata records) may support PATCH at their discretion.
====

[NOTE]
====
Implementations should document the intended access scope of the API (e.g., internal/LAN access only,
or publicly accessible). Publicly accessible APIs should apply appropriate authentication and rate
limiting in accordance with /req/data-api/auth.
====

[NOTE]
====
Implementations are encouraged to provide API documentation in OpenAPI (formerly Swagger) format,
which enables interactive documentation and client code generation. OpenAPI specifications should be
made available at a well-known endpoint (e.g. /api or /openapi.json).
====
...
|B a|The system SHOULD support XML as a response format, following a defined schema or profile, where
required for integration with legacy systems or national and regional networks.
|===
...
* Aggregation level (raw observations, or pre-computed official statistics where available)

|===

[NOTE]
====
On-demand statistical aggregation by the API (e.g., computing daily means from raw observations at
query time) is not recommended. Rigorous statistics require careful handling of data gaps, quality
flags, and defined computation periods. Pre-computed official statistics should be stored as standard
derived observations and served through the same query interface.
====
...
* BUFR for exchange with WMO systems, including CLIMAT and DAYCLI message types for monthly and daily summary data
|B a|Each export format SHALL be accompanied by documentation including:

* Description of the data structure and schema
...
[NOTE]
====
The documentation required in Statement B covers implementation-specific choices (e.g., which NetCDF
profile or JSON schema is used, how quality flags are encoded) that are not fully determined by the
format standard itself. Machine-readable schemas and format definitions should be made available at a
well-known endpoint referenced in the API documentation.
====

[NOTE]
====
For high-frequency or real-time data feeds, including full format documentation with every export
response may be impractical. In such cases, it is sufficient to make the format definition and schema
documentation available via a persistent, well-known endpoint referenced in the API documentation.
====